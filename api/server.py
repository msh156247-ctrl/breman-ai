...
from api.approval_notifications import normalize_approval_channels as _normalize_approval_channels
from api.approval_runtime import (
    APPROVAL_ALLOW_PRIVATE_WEBHOOKS_ENV,
    APPROVAL_EXTERNAL_CHANNEL_IDS,
    SMTP_FROM_ENV,
    SMTP_HOST_ENV,
    SMTP_PASSWORD_ENV,
    SMTP_PORT_ENV,
...
approval_runtime = ApprovalNotificationRuntime(
    outbox=approval_notification_outbox,
    decision_log=decision_log,
    manager=manager,
    get_channel_settings_for_mission=_approval_channel_settings_for_mission,
    decrypt_provider_key=decrypt_provider_key,
    is_truthy=_is_truthy,
    post_json_webhook_fn=lambda url, payload, token="": _post_json_webhook(url, payload, token),
    send_smtp_approval_email_fn=lambda notification, payload: _send_smtp_approval_email(notification, payload),
)


def _approval_channel_env_overrides() -> Dict[str, bool]:
    return approval_runtime.channel_env_overrides()


def _approval_notifications_for_gate(mission_id: str, task_id: str, gate_stage: str) -> List[Dict[str, Any]]:
    return approval_runtime.approval_notifications_for_gate(mission_id, task_id, gate_stage)


def _ensure_approval_notifications(gate: Dict[str, Any]) -> List[Dict[str, Any]]:
    return approval_runtime.ensure_approval_notifications(gate)


def _deliver_approval_notification(notification: Dict[str, Any]) -> Dict[str, Any]:
    return approval_runtime._deliver_approval_notification(notification)
...
def _resolve_approval_notifications(
    ...
    return approval_runtime.resolve_approval_notifications(mission_id, task_id, gate_stage, approved_by, timestamp)
...
def _retryable_approval_notifications(
    ...
    *,
    ...
    return approval_runtime.retryable_approval_notifications(
        mission_id,
        channels=channels,
...
def _recovered_pending_gate_from_log(mission_id: str, mission: Dict[str, Any]) -> Dict[str, Any] | None:
    return approval_runtime.recovered_pending_gate_from_log(mission_id, mission)


async def _dispatch_approval_notification_and_emit(notification: Dict[str, Any]) -> Dict[str, Any] | None:
    return await approval_runtime.dispatch_approval_notification_and_emit(notification)
