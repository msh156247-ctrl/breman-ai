...
def _workflow_runtime_providers(graph: Dict[str, Any] | None) -> set[str]:
...
def _validate_workflow_graph_shape(graph: Dict[str, Any] | None) -> None:
    if graph is None:
        return
    try:
...
async def create_mission(
    ...
    _validate_workflow_graph_shape(request.workflow_graph)
    ...
    if not request.use_mock:
        requested_providers = _workflow_runtime_providers(request.workflow_graph)
